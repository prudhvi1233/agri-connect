<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-100 leading-tight">Chat with {{ $contract->farmer->name }}</h2>
            <a href="{{ route('messages.index') }}" class="px-4 py-2 bg-gray-200 rounded-lg hover:bg-gray-300">← Back</a>
        </div>
    </x-slot>
        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="glass-card rounded-xl shadow-lg overflow-hidden" style="height: 70vh; display: flex; flex-direction: column;">
                ...
                <div class="bg-gradient-to-r from-blue-600 to-purple-600 text-white p-4">
                    <p class="font-bold">{{ $contract->farmer->name }} - {{ $contract->crop_name }}</p>
                ...
                <div class="flex-1 overflow-y-auto p-6 space-y-4 bg-transparent" id="messagesContainer">
                    @forelse($contract->messages as $message)
                        <div class="flex {{ $message->sender_id === auth()->id() ? 'justify-end' : 'justify-start' }}">
                            <div class="max-w-md">
                                <div class="{{ $message->sender_id === auth()->id() ? 'bg-gradient-to-br from-blue-600/80 to-purple-600/80 border border-blue-500/30 rounded-lg' : 'glass-card border' }} rounded-lg p-4 shadow-sm">
                                    <div class="flex items-center gap-2 mb-1">
                                        @if($message->sender_id !== auth()->id())
                                            <p class="text-xs font-bold {{ $message->sender_id === auth()->id() ? 'text-blue-100' : 'text-gray-300' }}">
                                                {{ $message->sender->name }}
                                            </p>
                                        @endif
                ...
                <div class="border-t p-4 glass-card">
                    <form method="POST" action="{{ route('messages.store', $contract) }}" enctype="multipart/form-data" class="space-y-3">
                        @csrf
